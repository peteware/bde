...
// convention classes (e.g., 'bbldcm::PeriodIcmaActualActual') to a run-time
// binding mechanism.
//
///Usage
///-----
...
class PeriodDayCountAdapter : public DayCountInterface {
  ...
  public:
    ...
    PeriodDayCountAdapter(const bsl::vector<bdlt::Date>& periodDate,
        ...
        // defines the years for each period (i.e. 0.25 for quarterly periods).
        // The behavior is undefined unless 'periodDate.size() >= 2' and the
        // values contained in 'periodDate' are unique and sorted from minimum
        // to maximum.
    ...
    double yearsDiff(const bdlt::Date& beginDate,
        ...
        // construction, 'min(beginDate, endDate) >= periodDate.front()',
        // 'max(beginDate, endDate) <= periodDate.back()', and neither
        // 'beginDate' nor 'endDate' is in the year 1752.  Note that reversing
